/* 
  ...
  Revision 0.5 - Option to set to first tab instead of last 2023-10-21
*/

// Global vars (Manifest v2 ONLY)
var tabNewPos = -1;
			...
			browser.storage.local.get("prefs").then((results) => {
				if (results.prefs != undefined){
					if (JSON.stringify(results.prefs) != '{}'){
						...
						tabNewPos = results.prefs.tabPos;
					}
				}
			}).then(() => {
				// Is there a delay?
				if (delaySeconds < 1){ // NO DELAY
					...
					browser.tabs.move(info.tabId, {index: tabNewPos}).catch((err) => {
...
function handleAlarm(alarmInfo) {
	browser.storage.local.get(alarmInfo.name).then((results) => {
		browser.tabs.move(results[alarmInfo.name], {index: tabNewPos}).catch((err) => {
			console.log('tabs.move() error: ' + err.message);
		});
	})
}
